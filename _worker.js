...
// @ts-ignore
//© https://t.me/freevlesstrojan
import { connect } from 'cloudflare:sockets';
const proxyListURL = 'https://raw.githubusercontent.com/bexnxx2/tes/main/p.txt';
let proxyIP;
...
export default {
  async fetch(request, ctx) {
    try {
      ...
      const bexx = request.headers.get('Host');

      if (!upgradeHeader || upgradeHeader !== 'websocket') {
        if (url.pathname === '/sub/clash') {
          const type = url.searchParams.get('type') || 'vless';
          const tls = url.searchParams.get('tls') === 'false' ? false : true || true;
          ...
          const bug = wildcard ? bexx : bugs;
          const configs = await generateClashSub(type, bug, bexnxx, tls);
          return new Response(configs, {
            headers: { 'Content-Type': 'text/plain' }
          });
        }
        if (url.pathname === '/sub/v2rayng') {
          const type = url.searchParams.get('type') || 'vless';
          const tls = url.searchParams.get('tls') === 'false' ? false : true || true;
          const wildcard = url.searchParams.get('wildcard') === 'true';
          const bugs = url.searchParams.get('bug') || bexx;
          const bexnxx = wildcard ? `${bugs}.${bexx}` : bexx;
          const bug = wildcard ? bexx : bugs;
          const configs = await generateV2rayngSub(type, bug, bexnxx, tls);
          return new Response(configs, {
            headers: { 'Content-Type': 'text/plain' }
          });
        }
...
async function generateClashSub(type, bug, bexnxx, tls) {
  const proxyListResponse = await fetch(proxyListURL);
  const proxyList = await proxyListResponse.text();
  const ips = proxyList.split('\n').filter(Boolean);
  const ipRegex = /(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/g;
  const ipss = ips.flatMap(line => {
  ...
  for (let ip of ipss) {
    ...
    const checkIspURL = `https://ipinfo.bexnxx.us.kg/api?ip=${ip}`;
    const ispResponse = await fetch(checkIspURL);
    const ispData = await ispResponse.json();
    const isp = ispData.isp;
    const cc = ispData.country;
    const ports = tls ? '443' : '80';
    const snio = tls ? `\n  servername: ${bexnxx}` : '';
    let ispName = `${isp} (${cc}) ${count ++}`;
    if (type === 'vless') {
      bex += `  - ${ispName}\n`
      conf += `
- name: ${ispName}
...
- name: ${ispName} trojan
  ...
  return `# https://t.me/freevlesstrojan
# t.me/bexnxx
port: 7890
socks-port: 7891
redir-port: 7892
...
async function generateV2rayngSub(type, bug, bexnxx, tls) {
  const proxyListResponse = await fetch(proxyListURL);
  const proxyList = await proxyListResponse.text();
  const ips = proxyList.split('\n').filter(Boolean);
  const ipRegex = /(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(\b25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/g;
  ...
  for (let ip of ipss) {
    ...
    if (type === 'vless') {
      if (tls) {
        conf += `vless://${UUIDS}\u0040${bug}:443?encryption=none&security=tls&sni=${bexnxx}&fp=randomized&type=ws&host=${bexnxx}&path=%2Fvl%3D${ip}#${isp}\n`;
      } else {
        conf += `vless://${UUIDS}\u0040${bug}:80?path=%2Fvl%3D${ip}&security=none&encryption=none&host=${bexnxx}&fp=randomized&type=ws&sni=${bexnxx}#${isp}\n`;
      }
    } else if (type === 'trojan') {
      if (tls) {
        conf += `trojan://${UUIDS}\u0040${bug}:443?encryption=none&security=tls&sni=${bexnxx}&fp=randomized&type=ws&host=${bexnxx}&path=%2Ftr%3D${ip}#${isp}\n`;
      } else {
        conf += `trojan://${UUIDS}\u0040${bug}:80?path=%2Ftr%3D${ip}&security=none&encryption=none&host=${bexnxx}&fp=randomized&type=ws&sni=${bexnxx}#${isp}\n`;
      }
    } else if (type === 'mix') {
      if (tls) {
        conf += `vless://${UUIDS}\u0040${bug}:443?encryption=none&security=tls&sni=${bexnxx}&fp=randomized&type=ws&host=${bexnxx}&path=%2Fvl%3D${ip}#${isp}\n`;
